...
const StateTable = ({
    ...
    setShowStateUTUpdateModal,
    setSelectedForDeleteStateUTId,
    setShowStateUTDeleteModal,
}) => {
    const columns = [
        { label: "S.No.", index: 0, sortable: true },
        { label: "Name", index: 1, sortable: true },
        { label: "Type", index: 2, sortable: true },
        { label: "Action", index: 3, sortable: false },
    ];
        ...
        <div>
                                ...
                                <tr key={index} className="text-gray-700 border-b-2 h-9">
                                    ...
                                    <td className="flex flex-col px-2 py-2 space-x-0 space-y-2 xl:flex-row xl:space-y-0 xl:space-x-2">
                                        <button
                                            ...
                                            onClick={() => {
                                                setUpdateStateUTId(state.id);
                                                setUpdateStateUTName(state.name);
                                                setUpdateStateUTType(state.type);
                                                setShowStateUTUpdateModal(true);
                                            }}
                                        >
                                            Update
                                        </button>
                                        <button
                                            className="px-2 py-1 text-white rounded bg-slate-800"
                                            onClick={() => {
                                                setSelectedForDeleteStateUTId(state.id);
                                                setShowStateUTDeleteModal(true);
                                            }}
                                        >
                                            Delete
                                        </button>
            ...
            <div className="block space-y-4 sm:hidden ">
                ...
                {isLoading ? (
                    ...
                    stateUTData.content.map((state, index) => (
                        <div key={state.id} className="flex flex-col p-4 text-gray-700 bg-white border rounded shadow">
                            <p className="font-Lexend_SemiBold">
                                {(currentPage - 1) * rowsPerPage + index + 1} . {state.name}
                            </p>
                            <p>
