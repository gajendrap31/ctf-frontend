...
export const State_UTTable = ({
    ...
    const columns = [
        ...
        { label: "Total Registered Users", index: 3, sortable: false },
                ...
                <tbody>
                    {isLoading ? (
                        ...
                        state.content.map((state, index) => (
                            <tr key={state.id} className="text-gray-700 border-b-2 h-9">
                                <td className="text-center">
                                    {(currentPage - 1) * rowsPerPage + index + 1}
                                </td>
                                <td className="ps-1">{state.name}</td>
                                <td className="ps-1">{state.type}</td>
                                <td className="ps-2">
                                ...
                                <td className="ps-1 ">
                                    <button className="px-2 py-1 text-white bg-gray-700 rounded"
                                        ...
                                        onClick={() => {
                                            ...
                                            setSelectedState_UT(state.id)
                                            setSelectedState_UTName(state.name)
                                        }}>
                                        View
                                    </button>
                                </td>
...
// MobileCardView.js
export const MobileState_UTCards = ({
    state,
    currentPage,
    rowsPerPage,
    setCurrentPage,
        ...
        {isLoading ? (
            ...
            state.content.map((state, index) => (
                <div key={state.id} className="flex flex-col p-4 text-gray-700 bg-white border rounded shadow">
                    ...
                    <p className=""><strong>Total Registered Users: </strong>{state.totalRegisteredUsers}</p>
                    <button
